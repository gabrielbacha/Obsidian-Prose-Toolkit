...
describe("external links", () => {
  ...
  it("uses the exact tracked website destination", () => {
    expect(WEBSITE_URL).toBe("https://www.gabrielbacha.com/?utm_source=obsidian_app&utm_medium=referral&utm_campaign=obsidian_assets&utm_content=prose_toolkit_settings");
  });

  it("uses preselected GitHub issue forms", () => {
    expect(FEATURE_REQUEST_URL).toBe("https://github.com/gabrielbacha/Obsidian-Prose-Toolkit/issues/new?template=feature_request.yml");
    expect(MORE_PLUGINS_URL).toBe("https://obsidian.md/plugins?search=Gabriel%20Bacha");
    expect(BUG_REPORT_URL).toBe("https://github.com/gabrielbacha/Obsidian-Prose-Toolkit/issues/new?template=bug_report.yml");
  });
});
